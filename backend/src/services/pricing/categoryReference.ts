 ...
 * The categories at the top of `REFERENCES` are the deterministic demo
 * categories we actually demonstrate (bangles, jute bag, pottery, textile,
 * saree). They carry realistic Indian-artisan price bands so the demo shows
 * credible numbers (e.g. bangles in the low hundreds, NOT ₹7,000+). These are
 * curated ESTIMATES, never "live" marketplace prices. Categories with no
 * matching reference resolve to `null` and the pricing service reports them
 * as unavailable ("बाज़ार मूल्य उपलब्ध नहीं है") so the artisan is never shown
 * a fabricated range.
 */
...
const REFERENCES: CategoryRefInternal[] = [
  // --- Curated DEMO categories (deterministic, realistic artisan prices) ---
  { category: 'bangles', baseMin: 150, baseMax: 900, categoryWords: ['bangles', 'bangle', 'chooda', 'kada'], fallbackWords: ['bangle', 'bangles', 'chooda', 'kada', 'choodi', 'churiyan', 'galaband'] },
  { category: 'jute bag', baseMin: 250, baseMax: 1500, categoryWords: ['jute'], fallbackWords: ['jute'] },
  { category: 'pottery', baseMin: 150, baseMax: 2500, categoryWords: ['pottery', 'ceramic', 'terracotta', 'clay'], fallbackWords: ['pot', 'vase', 'clay pot', 'terracotta', 'ceramic'] },
  { category: 'textile', baseMin: 300, baseMax: 6000, categoryWords: ['textile', 'handloom', 'khadi', 'fabric'], fallbackWords: ['textile', 'handloom', 'khadi', 'fabric', 'woven', 'weave'] },
  { category: 'saree', baseMin: 500, baseMax: 8000, categoryWords: ['saree', 'sari'], fallbackWords: ['saree', 'sari', 'silk saree'] },

  // --- Broader generic categories (kept wide for non-demo products) ---
  ...
  { category: 'jewellery', baseMin: 400, baseMax: 30000, categoryWords: ['jewellery', 'jewelry', 'necklace', 'earring', 'bracelet'], fallbackWords: ['necklace', 'earring', 'bracelet', 'mala', 'bali'] },
...
function score(ref: CategoryRefInternal, category: string, name: string, description: string): number {
  ...
  for (const w of ref.categoryWords) {
    if (cat && cat.split(/\s+/).some((tok) => tok.includes(w) || tok.startsWith(w))) {
...
/**
 * Match a free-text category name (and product name/description) to a
 * reference. Returns `null` when NOTHING matches so the pricing service can
 * report "मूल्य उपलब्ध नहीं है" instead of fabricating a general range.
 */
export function resolveCategoryReference(
  category: string,
  name = '',
  description = ''
): CategoryPriceReference | null {
